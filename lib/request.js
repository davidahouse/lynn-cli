...
function executeRequest(
  ...
  request.options.method =
    requestFile.method != null ? requestFile.method.toUpperCase() : "get";
  ...
  runner.execute(function(result) {
    ...
    if (autoSave == true) {
      ...
      // /log/<request without .json>/<status code>_<date time>.json
      const savePath = workingFolder + "/log/" + request.title + "/";
      const timestamp =
    ...
    callback(result, response, request);
  });
}
...
function constructPath(requestFile, environment) {
  ...
  if (queryStringStart > 0) {
    return templateReplace(
      requestPath.substring(0, queryStringStart),
      environment
    );
  } else {
...
function constructForm(requestFile, environment) {
  ...
  //     form = {fields: {}, files: {}}
  //     const properties =
  //        operation.request.requestBody.content['multipart/form-data'].schema.properties
 ...
 */
function gatherRequests(workingFolder) {
  const requests = {};
  const apiRootPath = files.rootPath(workingFolder, "requests");
  const apiFiles = files.listFiles(workingFolder, "requests");
  apiFiles.forEach(apiFile => {
    const apiContents = parseApiFile(apiRootPath + "/" + apiFile);
    requests[apiContents.requestId] = {
      kind: "request",
      file: apiFile,
      summary: apiContents.summary
    };
  });
  return requests;
}
...
function capture(apiFile, operationName, result) {
  ...
  //     if (operation.request.responses[responseKey]['x-capture'] != null &&
  ...
  //                operation.request.responses[responseKey]['x-capture'][key], result)
  //         }
  //       }
  //     }
  //   }
