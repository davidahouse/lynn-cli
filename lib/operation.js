...
function constructPath(apiFile, operation, environment) {
  ...
  if (queryStringStart > 0) {
    return templateReplace(operationPath.substring(0, queryStringStart), environment)
  } else {
...
function constructBody(operation, environment) {
  ...
  if (operation.request.requestBody != null && operation.request.requestBody.content != null) {
    if (operation.request.requestBody.content['application/json'] != null) {
      ...
      if (operation.request.requestBody.content['application/json'].schema['$ref'] != null) {
        const refContents = fs.readFileSync(operation.request.requestBody.content['application/json'].schema['$ref'])
        body = JSON.parse(refContents)
      } else {
        body = {}
        const properties = operation.request.requestBody.content['application/json'].schema.properties
        for (const name in properties) {
          if (properties.hasOwnProperty(name)) {
            if (name === '$ref') {
...
function gatherOperations(workingFolder, currentProject) {
  const operations = {}
  const apiRootPath = files.rootPath(workingFolder, 'operations', currentProject)
      ...
      if (apiContents.paths.hasOwnProperty(pathKey)) {
        ...
        for (const pathOperation in apiPath) {
          if (apiPath.hasOwnProperty(pathOperation)) {
            ...
            if (operationId != null) {
              operations[operationId] = {kind: 'operation', file: apiFile, summary: apiPath[pathOperation].summary} 
            }
          }
        }
      }
