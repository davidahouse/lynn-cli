...
function gatherOperations(workingFolder, currentProject) {
  const operations = {}
  const apiRootPath = files.rootPath(workingFolder, 'api', currentProject)
...
function capturedValue(jsonPointer, result) {
  ...
  try {
    if (jsonPointer.startsWith('$')) {
      ...
      found = jp.query(result.data, '$' + jsonPointer.substring(1))
    } else {
      found = ptr.get(result.data, jsonPointer)
    }
  } catch (e) {
