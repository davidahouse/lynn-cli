 ...
 */
function generateDocs(workingFolder, request, requestFile, currentProject) {
  return generateRequest(workingFolder, currentProject, request, requestFile)
}
 ...
 */
function generateReadme(workingFolder, currentProject, summaries) {
  const contents = []
  ...
  files.createFile(workingFolder, currentProject, 'docs', 'README.md', contents.join('\n'))
}
 ...
 */
function generateRequest(workingFolder, currentProject, request, requestFile) {
  ...
  if (requestPath.request.parameters) {
    requestPath.request.parameters.forEach((param) => {
      if (param.in == 'header') {
        contents.push('| ' + param.name + ' | ' + param.schema.default + ' | ' + (param.required ? 'Required' : '') + ' ' + param.schema.type + ' | ')
      }
    })    
  }
  contents.push(' ')
  ...
  files.createFile(workingFolder, currentProject, 'docs', request + '.md', contents.join('\n'))

  // Return a summary for the README.md
  return '| [' + request + '](' + request + '.md) | ' + requestPath.path + ' | ' + requestPath.request.summary + ' | '
}
